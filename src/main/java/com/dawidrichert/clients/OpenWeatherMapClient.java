...
public class OpenWeatherMapClient {
    ...
    private static final String currentWeatherUrlFormat = "http://api.openweathermap.org/data/2.5/forecast/city?APPID=" + APPID + "&q=%s&units=metric&lang=pl";

    public CurrentWeatherData getCurrentWeather(String city) {
        if(Arrays.asList(Config.listOfAllowedCities).contains(city)) {
            try {
                String json = readUrl(String.format(currentWeatherUrlFormat, city));
